...
class UTIL_DateTime
{
    ...
    {
        $simpleDateFormat =  OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_RENDER_FORMAT_DATE_FIELD, array('timeStamp' => $timeStamp, 'onlyDate' => $onlyDate)));
        if($simpleDateFormat->getData() && isset($simpleDateFormat->getData()['jalaliSimpleFormat'])){
            return $simpleDateFormat->getData()['jalaliSimpleFormat'];
        }
        return  self::gregorianFormatSimpleDate( $timeStamp, $onlyDate);
    }

    public  static function gregorianFormatSimpleDate( $timeStamp, $onlyDate = false )
    {
        ...
        if ( $onlyDate )
        {
            return ( date('y', $timeStamp) === date('y', time()) ) ?
                $month . strftime(" %#d", $timeStamp) :
                $month . strftime(" %#d '%y", $timeStamp);
        }

        return ( date('y', $timeStamp) === date('y', time()) ) ?
             $month . ( $militaryTime ? strftime(" %#d, %H:%M", $timeStamp) : strftime(" %#d, %I:%M%p", $timeStamp) ) :
             $month . ( $militaryTime ? strftime(" %#d '%y, %H:%M", $timeStamp) : strftime(" %#d '%y, %I:%M%p", $timeStamp) );
    }
...
//               return $dateTime;
    ...
    { $simpleDateFormat =  OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_RENDER_FORMAT_DATE_FIELD, array('year' => $year, 'month' => $month , 'day' => $day , 'isBirthday' => true)));
        if($simpleDateFormat->getData() && isset($simpleDateFormat->getData()['jalaliSimpleFormat'])){
            return $simpleDateFormat->getData()['jalaliSimpleFormat'];
        }
        return  self::gregorianFormatBirthdate( $year, $month, $day);
    }
    public static function gregorianFormatBirthdate( $year, $month, $day )
    {
