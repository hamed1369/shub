        ...
        */
        ...
        if ( !empty($idList) )
        {
            ...
            foreach ( $list as $key => $item )
            {
                ...
                if ( mb_strlen($item->getEntry()) > 100 )
                {
                    $event = OW::getEventManager()->trigger(new OW_Event(IISEventManager::PARTIAL_HALF_SPACE_CODE_DISPLAY_CORRECTION, array('sentence' => $item->getEntry(), 'trimLength' => 100)));
                    if(isset($event->getData()['correctedSentence'])){
                        $sentence = $event->getData()['correctedSentence'];
                        $list[$key]->setEntry($sentence.'...');
                    }
                    else{
                        $list[$key]->setEntry(UTIL_String::truncate($item->getEntry(), 100, '...'));
                    }
                }
