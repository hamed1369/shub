...
class BASE_CMP_OembedAttachment extends OW_Component
{
    ...
    {
        $stringRenderer = OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_AFTER_NEWSFEED_STATUS_STRING_READ,array('string' => $this->oembed['url'])));
        if(isset($stringRenderer->getData()['string'])){
            $this->oembed['url'] = $stringRenderer->getData()['string'];
        }
        $stringRenderer = OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_AFTER_NEWSFEED_STATUS_STRING_READ,array('string' => $this->oembed['href'])));
        if(isset($stringRenderer->getData()['string'])){
