...
class OW_Application
{
    ...
    public function init()
    {
        ...
        if ( OW::getUser()->isAuthenticated() )
        {
            OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_BEFORE_UPDATE_ACTIVITY_TIMESTAMP));
        ...
        $viewRenderer->assignVar('isAuthenticated', OW::getUser()->isAuthenticated());
         $viewRenderer->assignVar('bottomPoweredByLink', '<a href="http://www.shub.ir/" target="_blank" title="'.OW::getLanguage()->text('base','powered_by_community').'"><img src="' . $currentThemeImagesDir . 'powered-by-motoshub.png" /></a>');
